...
| Mails | Seulement les trois messages adressés à propos d'un vœu ; le reste est le chantier C2 |
...
| `statut` | text | `propose`, `valide`, `refuse` |
...
Un conflit n'est pas stocké, il se lit : plusieurs vœux en `propose` ou `valide` sur la même `(salle, jour, moment)`. Un vœu `refuse` ne compte plus dans le conflit — c'est ce qui permet de le fermer.

**Une case ne porte qu'un seul vœu validé.** Valider un vœu sur une case qui en a déjà un lève `AGENDA_CASE_PRISE`, et l'écran dit lequel occupe la place. C'est la seule contrainte que l'outil impose au gérant : sans elle, « validé » ne garantit plus une salle à personne, et l'arbitrage n'a servi à rien.

Chacun **supprime ses propres vœux quel que soit leur état**, y compris validés : se retirer d'un créneau ne demande l'accord de personne. La suppression emporte le fil de commentaires.
...
|---|---|---|
| `agenda_voeux` | toute l'équipe : la vision d'ensemble est le but | chacun crée et supprime les siens ; seuls les gérants changent `statut`, `decide_par`, `decide_le` |
...
Trois messages, tous adressés à une personne précise, tous envoyés par la file existante et la route de cron du Klub, dans une table de file distincte `agenda_mails` de même forme que `klub_mails`.
...
1. **Scénarios SQL** (`supabase/tests/agenda.sql`, transaction annulée) : un praticien crée un vœu et le supprime, même validé ; il ne change pas le `statut` d'un vœu, ni du sien ni d'un autre ; il ne supprime pas le vœu d'un autre ; il commente le sien et pas celui d'un autre ; un gérant décide et commente n'importe lequel ; valider une case déjà prise lève `AGENDA_CASE_PRISE` ; modifier un vœu validé le ramène en `propose` ; `anon` ne lit rien.
