...
declare
  ...
  v_rang integer;
begin
  if char_length(trim(coalesce(p_prenom, ''))) = 0 or char_length(trim(coalesce(p_nom, ''))) = 0 then
  ...
  if found then
    ...
    values (p_seance, left(trim(p_prenom), 60), left(trim(p_nom), 60), v_email, v_tel,
